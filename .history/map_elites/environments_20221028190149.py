...
class Architext(BaseEnvironment):
    ...
    # Record different definitions of behaviour spaces in a dict. Feel free to add.
    behaviour_mode_spec = {'heat_loss_form_factor':
                               {'genotype_ndim': 1,
                                'behaviour_space_fn': _hlff},
                            'floor_area_entropy':
                               {'genotype_ndim': 1,
                                'behaviour_space_fn': _gfa_entropy}}

    def __init__(self, seed: str, config: Union[str, dict, DictConfig], target_layout: str, func_name: str, prompts: list, 
                 inference_server='localhost:5000', behaviour_mode: float = ['heat_loss_form_factor', 'floor_area_entropy']):
        """
        Parameters:
            ...
            func_name: the name of the function to be called to return layouts.
            inference_server: (Optional) the address of the inference server: 'domain:port'.
        """
        self.seed = seed
        if isinstance(config, str):
            self.config = OmegaConf.load(config)
        ...
        self.behaviour_mode = behaviour_mode
        self.genotype_hllf_ndim = self.behaviour_mode_spec[self.behaviour_mode][0]['genotype_ndim']
        self.genotype_gfa_entropy_ndim = self.behaviour_mode_spec[self.behaviour_mode][1]['genotype_ndim']
        self.genotype_hllf_space = np.arange(0, 3.25, 0.25) # bins=12
        self.genotype_gfa_entropy_space = np.arange(0, 1.25, 0.1) # bins=10
        #map_dim = 12 x 10 = 120

        self.inference_server = inference_server

    def sample_layouts(self, **kwargs) -> List[Genotype]:
        """
        Sample layouts from the model by randomly selecting prompts
        Returns:
            the generated layout in a string representation.
        """
        return self._get_layout(self.seed + self.def_and_docstring, **kwargs)

    def mutate_by_fim(self, x: Genotype, **kwargs) -> List[Genotype]:
        """
        Randomly mutate a batch of codes from a given individual and evaluate their outputs.
        Parameters:
            x: the individual to be mutated.
        Returns:
            a tuple of the code string and the returning result (None if there is error).
        """
        return self._get_fim_completion(self.def_for_mutation + x[0] + self.def_and_docstring, **kwargs)

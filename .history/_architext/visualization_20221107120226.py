...
def plot_map(data, min_qual, max_qual, tick_length, , xlabels, ylabels, linewidths=1.5, linecolor='black', grid_kws = {"width_ratios": (.9, .05), "wspace": .05}, 
    ...
    g.set_xlabel('Comfortable Open Space (%)', fontsize=fontsize)
    g.set_ylabel('Dangerous Open Space ($m^2$)', fontsize=fontsize, labelpad=20)
    ax.invert_yaxis()
    fig.tight_layout()

    if(save_folder):
        fig.savefig(save_folder + '\\GridPotential.png')
        fig.savefig(save_folder + '\\GridPotential.png', dpi=800)

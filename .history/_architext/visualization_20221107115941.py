...
def plot_map(data, min_qual, max_qual, tick_length, , xlabels, ylabels, linewidths=1.5, linecolor='black', grid_kws = {"width_ratios": (.9, .05), "wspace": .05}, 
             cbar_kws={"orientation": "vertical"}, cmap_reversed = matplotlib.cm.get_cmap('YlGnBu_r'), figsize=(22,20), annotation=True, 
             annot_kws={"fontsize": 18}, labelsize=40):
    ...
    g = sns.heatmap(data, vmin=min_qual, vmax=max_qual, ax=ax, cbar_ax=cbar_ax, cbar_kws=cbar_kws, linewidths=linewidths, 
                linecolor=linecolor, cmap=cmap_reversed, annot=annotation, annot_kws=annot_kws, fmt=".0f")
...
ax.set_xticklabels(xlabels, fontsize=80)
ax.set_yticklabels(ylabels, fontsize=80)

cbar_ax.tick_params(labelsize=80)
g.set_xlabel('Comfortable Open Space (%)', fontsize=80)
g.set_ylabel('Dangerous Open Space ($m^2$)', fontsize=80, labelpad=20)
ax.invert_yaxis()
fig.tight_layout()
#fig.savefig(plots_folder + '\\GridPotential_MIT_Fitness_{}.png'.format(attribute))
#fig.savefig(plots_folder + '\\GridPotential_{}_{}_{}.png'.format(cases[0], experiment, attribute), dpi=800)